...
class Stitcher:
    def __init__(self, calibrations, reprojection_viewpoint, masks, min_dist, max_dist, 
        ...
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)),'vec_utils.cuh'), 'r', encoding='utf-8') as f:
            # with open(os.path.join(os.path.abspath(__file__), 'vec_utils.cuh'), 'python/vec_utils.cuh', 'r') as f
            utils_source = f.read()
        with open(os.path.join(os.path.dirname(os.path.abspath(__file__)),'stitcher.cu'), 'r', encoding='utf-8') as f:
