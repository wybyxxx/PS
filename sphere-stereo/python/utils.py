...
#  opencv读取exr出问题，因此修改为openexr库进行读取
def read_exr(filename):
    ...
    # 合并通道为 RGB 图像
    gt_distance = np.stack(images, axis=-1)
    return gt_distance
...
def evaluate_rgbd_panorama(rgbd_panoramas, filename, dataset_path, bad_px_ratio_thresholds, panorama_resolution):
    ...
    try:
        ...
        #  opencv读取exr出问题，因此修改为openexr库进行读取
        gt_distance = read_exr(os.path.join(dataset_path, "gt/inv_distance_" + read_name + ".exr"))
...
# 新增方法（opencv保存exr文件报错）
def save_exr(filename, image_data):
    # 获取图像数据的宽度和高度
    ...
    if len(image_data.shape) == 3:
        # 如果是多通道图像，需要为每个通道准备数据
        channels = ['R', 'G', 'B']
        channel_data = [image_data[:,:,i] for i in range(3)]
    else:
        # 如果是单通道图像，则只有一个通道
        channels = ['R']  # 假设单通道图像保存到红色通道
        channel_data = [image_data]
    # 将numpy数组转换为OpenEXR可以理解的数据格式
    channel_data_exr = [ch.astype(np.float32).tobytes() for ch in channel_data]
    # 写入EXR文件
    exr_file.writePixels(dict(zip(channels, channel_data_exr)))
    # 关闭文件
    exr_file.close()
...
def save_rgbd_panorama(rgbd_panoramas, filename, dataset_path):
    try:
        ...
        # 修改后代码（opencv保存exr文件报错）
        save_exr(os.path.join(dataset_path, "output/inv_distance_" + save_name + ".exr"), rgbd_panorama["inv_distance"])
